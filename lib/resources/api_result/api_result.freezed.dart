...
class _$ApiResultTearOff {
  ...
  Success<T> success<T>({List<T> data = const []}) {
    return Success<T>(
      data: data,
    );
  }
  ...
  Failure<T> failure<T>(
      {NetworkExceptions error = const NetworkExceptions.badRequest()}) {
    return Failure<T>(
      error: error,
    );
  }
...
class _$Success<T> with DiagnosticableTreeMixin implements Success<T> {
  const _$Success({this.data = const []});
...
abstract class Success<T> implements ApiResult<T> {
  const factory Success({List<T> data}) = _$Success<T>;
...
class _$Failure<T> with DiagnosticableTreeMixin implements Failure<T> {
  const _$Failure({this.error = const NetworkExceptions.badRequest()});

  @JsonKey(defaultValue: const NetworkExceptions.badRequest())
  @override
